...
def _(mo):
    ...
    This dataset is not yet on the IGVF portal; all data is read locally from `data/10X_multiome_15_timepoints/channel1/`.
    """)
    return
...
def _(mo):
    ...
    This dataset is not yet on the IGVF portal, so there is no accession-based download step here. The RNA counts (kallisto/bustools h5ad) and ATAC fragments were produced locally by the pipeline in `analyses/10x_multi_15_timepoints/README.md` and already live under `data/10X_multiome_15_timepoints/channel1/`.
    """)
    return
...
def _(Path, project_root):
    ch1_data_root_path = Path(project_root / "data/10X_multiome_15_timepoints/channel1/")

    ch1_outdir_root = Path(project_root / "results/10X_multiome_15_timepoints/channel1")
...
def load_h5_counts(
    ...
    try:
        _h5ad_path = ch1_data_root_path / "h5ad/10x_15timepoints_channel1.h5ad"
...
@app.cell
def initialize_cmo(ad, adata_flt, ch1_data_root_path, pd, project_root):
    ...
    _cmo_design_ch1 = _cmo_design[_cmo_design["Channel"] == "channel1"]
    cmo_to_timepoint = dict(zip(_cmo_design_ch1["CMO"], _cmo_design_ch1["Timepoint"]))
    ...
    # channel2's design -- see the README). Drop CMOs this channel does not use
    # before CLR/threshold/assignment ever sees them: a background-only column
    # still has a near-zero 95th-percentile threshold, so a handful of noise
    # counts can spuriously call it "positive" and break the CMO -> timepoint
    # lookup downstream.
    adata_cmo = adata_cmo[:, adata_cmo.var["gene_name"].isin(cmo_to_timepoint)].copy()
...
def cmo_clr_normalization(adata_cmo, np):
    ...
    # features, then takes exp. Generalized to adata_cmo.n_vars (23 for this
    # channel) instead of a hardcoded feature count, since it differs from the
    # 5-timepoints dataset's 25 CMOs.
...
def find_cmo_thresholds(clr, np):
    # All 23 CMOs showed clean, similarly-shaped bimodal CLR distributions (see
    # the per-tag GMM diagnostic that used to sit here) -- this dataset is much
    # less noisy than the mcginnis dataset that motivated the per-tag GMM
    # approach, so a simple global CLR quantile threshold is sufficient here.
...
def cmo_doublet_pair_confounding(
    ...
    **CMO doublet pairing: {_n_doublets:,} doublets with exactly two positive CMOs,
    ...
    do not reduce to a single CMO pair.
    ...
    **{_top5_share_null.mean():.1%}** null mean (1,000-permutation shuffle of which CMO-hit
    ...
    # Channel1's own timepoint labels (15 distinct stages), not the 5-timepoints
    # dataset's fixed d0-d4 -- see the timepoint_palette cell.
...
def strict_qc_cascade(
    ...
    def _run_strict_qc(adata, doublet_dominated_clusters):
        ...
        # silently let Negatives through.
        adata.obs["pass_cmo_singlet"] = adata.obs["cmo_status_scanpy"] == "Singlet"
        return True
    ...
    _steps = [
        ...
        ("CMO hashing status == Singlet", adata_flt.obs["pass_cmo_singlet"]),
...
def import_atac_fragments(
    ...
    atac_adata = snap.pp.import_fragments(
        ch1_data_root_path / "fragments/10x_15timepoints_channel1.fragments.tsv.gz",
