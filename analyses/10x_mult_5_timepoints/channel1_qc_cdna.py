...
def _(pd):
    ...
    old_adata_cmo_assignments_ch1 = old_adata_cmo_assignments[
        old_adata_cmo_assignments["barcode"].str.endswith("-1_1")
    ].copy()

    # build cleaned barcode
    old_adata_cmo_assignments_ch1["cleaned_barcode"] = (
        old_adata_cmo_assignments_ch1["barcode"]
        .str.replace(r"-1_1$", "", regex=True)
        + "_10x_5timepoints_channel1"
    )

    old_adata_cmo_assignments_ch1.reset_index(drop=True, inplace=True)
    old_assignments_df = (
        old_adata_cmo_assignments_ch1
...
@app.cell
def _(adata):
    adata.obs[["old_annotation"]].value_counts()
    return
...
@app.cell
def _(adata):
    adata.obs.loc[adata.obs["old_annotation"] != "Unassigned", "total_counts"].hist(bins=100)
    return


@app.cell
def _(adata):
    sum(adata.obs.loc[adata.obs["old_annotation"] != "Unassigned", "total_counts"]<700)
    return
...
@app.cell
def _(raw):
    sum(raw.obs["doublet_score"] < 0.15)
    return
...
def _(ad, adata, barcodes, gene_names, genes, mat, pd):
    ...
    cells_in_use = adata.obs_names.intersection(adata_cmo.obs_names.to_list())[adata.obs["doublet_score"] < 0.15]
...
@app.cell
def _(adata_flt):
    adata_flt.obs["timepoint_scanpy"]=="Negative"
    return
