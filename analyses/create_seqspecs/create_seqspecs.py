...
def _(conn, input_file_sets_by_channel):
    ...
    for (_ch, _set_acc), _info in fastq_metadata.items():
        ...
        for _f in _info["files"]:
            print(f"  {_f.get('accession')}  read={_f.get('illumina_read_type')}  fc={_f.get('flowcell_id')}  lane={_f.get('lane')}  run={_f.get('sequencing_run')}")
...
def _(conn, fastq_metadata):
    ...
    for _channel in sorted(summary):
        for _modality, _files in summary[_channel].items():
            _reads = sorted(set(_f["read"] for _f in _files if _f["read"] is not None))
...
def _(
    ...
    def _group_suffix(flowcell, lane, index, run):
        _parts = [flowcell or "unknown"]
        if lane is not None:
            _parts.append(f"l{lane}")
        if index is not None:
            _parts.append(f"i{index}")
    ...
    for (_ch_key, _set_acc), _info in fastq_metadata.items():
        ...
        _channel = _ch_key.split("_")[0]

        _run_groups = _defaultdict(lambda: _defaultdict(list))
        for _f in _info["files"]:
            ...
            _gkey = (
                ...
                _f.get("lane"),
                _f.get("index"),
                _f.get("sequencing_run"),
            )
            _run_groups[_gkey][_rt].append(_seq_file_ctx(_f))

        for _gkey, _by_read in _run_groups.items():
            _flowcell, _lane, _index, _run = _gkey
            _sfx = _group_suffix(_flowcell, _lane, _index, _run)
            ...
            if _assay == "single-nucleus RNA sequencing assay":
                _ctx["assay_id"] = f"{_channel}_gex_{_set_acc}_{_sfx}"
                ...
                _ctx["assay_id"] = f"{_channel}_cmo_{_set_acc}_{_sfx}"
                _ctx.update(_gex_bc_ctx)
            ...
            _gz_path = _seqspec_dir / f"{_ctx['assay_id']}.yaml.gz"
            _gz_path.write_bytes(_gz_buf.getvalue())

            _group_file_accs = [
                _f["file_id"]
                for _read_files in _by_read.values()
                for _f in _read_files
            ]

            seqspec_paths[_ctx["assay_id"]] = {
                "path":         _gz_path,
                "set_acc":      _set_acc,
                "info":         _info,
                "file_accs":    _group_file_accs,
                "group_suffix": _sfx,
            }
            print(f"written: {_gz_path}  ({len(_group_file_accs)} files)")
...
def _(hashlib, json, project_root, seqspec_paths):
    ...
    def _config_alias(file_set_alias, group_suffix):
        ...
        for prefix in ("measurement_", "auxiliaryset_", "curated_set_"):
            if prefix in file_set_alias:
                ...
                _base = f"{lab}:configuration_file_{rest.replace(prefix, '', 1)}"
                break
        return f"{_base}_{group_suffix}"

    _rows = []
    for _assay_id, _entry in seqspec_paths.items():
        _gz_path = _entry["path"]
        _set_acc = _entry["set_acc"]
        _info    = _entry["info"]
        _file_accs   = _entry["file_accs"]
        _group_suffix = _entry["group_suffix"]
        _md5 = hashlib.md5(_gz_path.read_bytes()).hexdigest()
        _rows.append({
            "aliases":             json.dumps([_config_alias(_info["alias"], _group_suffix)]),
            "award":               _info["award"],
            "lab":                 _info["lab"],
            "md5sum":              _md5,
            "file_format":         "yaml",
    ...
    print(f"Wrote {len(_rows)} rows to {manifest_tsv}")
    manifest_tsv
    return
